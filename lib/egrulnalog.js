...
const EGRUL_NALOG_RU = require('./egrulnalog.const');
const config = require('./egrulnalog.config');
 ...
 */
function get_egrul_raw(ogrninn, ul, captcha, token) {
	var d = vow.defer();
...
function get_egrul(ogrninn, ul, captcha, token) {
	...
	res => {
		...
		fs.writeFile(path.join(config.PDF_STORAGE_PATH ,pdf_file_name), res.r, 'binary', err => {
			if (err)
				return d.reject(err, res);

			egrulpp.parse(pdf_file_name, path.normalize(config.PDF_STORAGE_PATH))
			.then(
			res => d.resolve(res),
			err => d.reject(err)
			);
...
var c = undefined
var c = 934061
var t = '9AD79D007E689E5FA0224FD591EC4BB6599E9A257E81722F04C6EA7C64FBD2CAB1276140E831A6527F88CD98D9BA47EF'
...
{
get_egrul('713601861245', false, c, t).then(
res => console.log(res),
err => console.log(err)
);
}
